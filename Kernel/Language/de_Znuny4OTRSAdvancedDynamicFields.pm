...
sub Data {
    ...
    $Self->{Translation}->{'All views'}                       = 'Alle Oberflächen';

    # Filter
    $Self->{Translation}->{'Filter DynamicField Screen'}      = 'Oberflächen für Dynamische Felder filtern';
    $Self->{Translation}->{'Filter available fields'}         = 'Verfügbare Felder filtern';
    $Self->{Translation}->{'Filter assigned fields'}          = 'Zugewiesene Felder filtern';
    $Self->{Translation}->{'Filter assigned required fields'} = 'Zugewiesene Pflichtfelder filtern';
